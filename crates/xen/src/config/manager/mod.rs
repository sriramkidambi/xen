...
fn copy_directory_recursive(src: &PathBuf, dst: &PathBuf) -> Result<()> {
    if !src.exists() {
        return Ok(());
    }
...
impl ProfileManager {
    ...
    pub fn create_from_profile_with_resources(
        ...
        let source_profile_name = ProfileName::new(source_name)
            .map_err(|_| Error::InvalidProfileName(source_name.to_string()))?;

        let source_path = self.profile_path(harness, &source_profile_name);
        if !source_path.exists() {
            return Err(Error::ProfileNotFound(source_name.to_string()));
        }

        let target_path = self.create_profile(harness, target_name)?;

        // Copy all files from source profile to target profile
...
mod tests {
    ...
    fn setup_test_env(temp: &TempDir) -> TestEnvGuard {
        ...
        let xen_config_dir = temp.path().join("bridle_config");
