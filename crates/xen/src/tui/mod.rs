...
impl App {
    fn new() -> Result<Self, Error> {
        ...
        // Only show installed harnesses
        let mut harnesses: Vec<_> = HarnessKind::ALL
            .iter()
            .filter(|kind| Harness::new(**kind).is_installed())
            .copied()
            .collect();

        // If no harnesses are installed, show all with a message
        if harnesses.is_empty() {
            harnesses = HarnessKind::ALL.to_vec();
        }

        // If a default harness is configured, move it to position 0
        if let Some(default_id) = xen_config.default_harness()
...
fn render_harness_pane(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    // Check if any harnesses are actually installed
    let has_installed = app.harnesses.iter().any(|kind| Harness::new(*kind).is_installed());

    if !has_installed {
        // Show empty state when no harnesses are installed
        let empty_state = widgets::EmptyState::new(
            "Harnesses",
            vec![
                "No AI harnesses installed".to_string(),
                "".to_string(),
                "Install a harness to get started:".to_string(),
                "  claude-code  - Claude Code".to_string(),
                "  opencode    - OpenCode".to_string(),
                "  goose       - Goose AI".to_string(),
                "  amp-code    - Amp Code".to_string(),
                "  copilot-cli - GitHub Copilot CLI".to_string(),
                "  crush       - Crush CLI".to_string(),
                "".to_string(),
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|kind| {
            ...
            ListItem::new(format!("{} {}", indicator, harness.kind())).style(Style::default())
        })
        .collect();
